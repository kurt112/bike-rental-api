...
public interface BikeRepository extends JpaRepository<Bike, Long> {
    @Query(value = "SELECT t FROM Bike t WHERE t.status = ?1 and (t.assignedCustomer.user.lastName like %?2% or t.assignedCustomer.user.firstName like %?2% or t.name like %?2% or t.description like %?2% or t.name like %?2% or t.brand like %?2%)")
